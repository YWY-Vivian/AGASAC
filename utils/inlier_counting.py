...
def soft_inlier_fun_gen(beta, tau, inverse=False, use_approx=False):  
    def f(d, w):
        mask = (d > tau).float()
        scaled = beta * d * w / tau - beta

        if use_approx:
            result = 0.5 + 0.25 * scaled.clamp(-2, 2)  
        else:
            result = torch.sigmoid(scaled)

        result = 1 - result if not inverse else result
...
def count_inliers(opt, residuals, log_weights):
    ...
    inlier_scores = inlier_fun(residuals, weights_e) 
    ...
    inlier_ratios = inlier_scores.sum(-1) * 1.0 / N
